...
/**
 * @brief parse command line options for scancode toolkit to get required flags
 * for scanning Enhanced to support parallel processing arguments
 * @param[in] argc                     command line arguments count
 * @param[in] argv                     command line argument vector
 * @param[out] cliOption               command line options for scancode toolkit
 * @param[out] ignoreFilesWithMimeType set this flag is user wants to ignore
 * FilesWithMimeType
...
bool parseCommandLine(int argc, char** argv, string& cliOption,
                      ...
                      int parallelParams[5])
{
  parallelParams[0] = 1;     // parallel processes
  parallelParams[2] = 10;    // nice level
  parallelParams[3] = 1000;  // max tasks per worker
  parallelParams[4] = 60;    // heartbeat interval
