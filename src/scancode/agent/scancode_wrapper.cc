 ...
 *
 * using cli command for custom template with parallel processing support
 *
 * @param state  an object of class State which can provide agent Id and
 * CliOptions
...
{
  string projectUser =
      fo_config_get(sysconfig, "DIRECTORIES", "PROJECTUSER", NULL);
  string cacheDir = fo_config_get(sysconfig, "DIRECTORIES", "CACHEDIR", NULL);
